class PagesController < ApplicationController
    ...
    def search
      if params[:search].present? && params[:search].strip != ""
         session[:ccasa_search] = params[:search]
      end
      
      arrResult = Array.new
      
      if session[:ccasa_search] && session[:ccasa_search] != ""
         @rooms_address = Room.where(active: true).near(session[:ccasa_search], 5, order:'distance')
      else
